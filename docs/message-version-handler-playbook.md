...
The `MessageVersionHandler` is **only invoked** when both conditions are true:
...
(`build_tell_envelope`, `build_ask_envelope`) accept a `version`
parameter — make sure you pass it.

```rust
// ❌ Wrong — version is None, handler will never run
let envelope = build_tell_envelope(target, name, &msg, headers, None, None)?;

// ✅ Correct — version is set, handler runs on mismatch
let envelope = build_tell_envelope(target, name, &msg, headers, None, Some(2))?;
```
...
**Best practice:** use a stable, versioning-friendly name for
`message_type`, not the Rust type path. For example:

```rust
fn message_type(&self) -> &'static str {
    "myapp::orders::PlaceOrder"  // stable string, not std::any::type_name
}
```

If you rename the Rust struct, keep the `message_type()` string unchanged.
...
```rust
use dactor::remote::build_tell_envelope;

let envelope = build_tell_envelope(
    target_id,
    "order-processor",
    &place_order_msg,
    headers,
