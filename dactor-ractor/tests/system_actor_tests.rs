...
#[tokio::test]
async fn sa1_spawn_unknown_type_fails() {
    let mut runtime = RactorRuntime::new();
    let request = SpawnRequest {
...
#[tokio::test]
async fn sa1_multiple_spawns_get_unique_ids() {
    let mut runtime = RactorRuntime::new();
    runtime.register_factory("test::Actor", |_| Ok(Box::new(())));
...
#[tokio::test]
async fn sa2_runtime_has_watch_manager() {
    let runtime = RactorRuntime::new();
    assert_eq!(runtime.watch_manager().watched_count(), 0);
}

#[tokio::test]
async fn sa2_remote_watch_and_notify() {
    let mut runtime = RactorRuntime::new();
    let target = ActorId {
...
#[tokio::test]
async fn sa2_multiple_watchers() {
    let mut runtime = RactorRuntime::new();
    let target = ActorId {
        node: NodeId("node-1".into()),
        local: 1,
...
#[tokio::test]
async fn sa3_runtime_has_cancel_manager() {
    let runtime = RactorRuntime::new();
    assert_eq!(runtime.cancel_manager().active_count(), 0);
}

#[tokio::test]
async fn sa3_register_and_cancel() {
    let mut runtime = RactorRuntime::new();
...
#[tokio::test]
async fn sa3_cancel_unknown_request() {
    let mut runtime = RactorRuntime::new();
...
#[tokio::test]
async fn sa4_runtime_has_node_directory() {
    let runtime = RactorRuntime::new();
    assert_eq!(runtime.node_directory().peer_count(), 0);
}

#[tokio::test]
async fn sa4_connect_and_disconnect_peer() {
    let mut runtime = RactorRuntime::new();
...
#[tokio::test]
async fn sa4_connect_without_address() {
    let mut runtime = RactorRuntime::new();
...
#[tokio::test]
async fn sa1_spawn_returns_actor_for_caller_to_use() {
    let mut runtime = RactorRuntime::new();
...
#[tokio::test]
async fn sa1_malformed_bytes_returns_error() {
    let mut runtime = RactorRuntime::new();
    runtime.register_factory("test::Counter", |bytes: &[u8]| {
...
#[tokio::test]
async fn sa4_reconnect_preserves_address() {
    let mut runtime = RactorRuntime::new();
    let peer = NodeId("peer-1".into());
...
#[tokio::test]
async fn sa4_reconnect_updates_address() {
    let mut runtime = RactorRuntime::new();
    let peer = NodeId("peer-1".into());
...
#[tokio::test]
async fn sa2_unwatch_nonexistent_is_noop() {
    let mut runtime = RactorRuntime::new();
...
#[tokio::test]
async fn sa3_double_cancel_returns_not_found() {
    let mut runtime = RactorRuntime::new();
    let token = CancellationToken::new();
    runtime.register_cancel("req-1".into(), token);
