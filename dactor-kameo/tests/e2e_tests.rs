...
async fn t5_kameo_stop_notification() {
    ...
    let stop_resp = cluster.stop_actor("watch-node", "target").await.unwrap();
    ...
    let ask_resp = cluster
        ...
        .await
        .unwrap();
    assert!(!ask_resp.success, "actor should be gone after stop");
...
async fn e2e_node_crash_detection() {
    ...
    // Launch 2 nodes
    let mut cluster = TestCluster::builder()
        .node("alive", &binary, &[], 50105)
        .node("doomed", &binary, &[], 50106)
        .build()
        .await;
...
async fn e2e_ask_timeout_cancels_slow_handler() {
    ...
    let mut cluster = TestCluster::builder()
        .node("timeout-cancel", &binary, &[], 50135)
        .build()
        .await;
