...
async fn try_connect_peer_accepted_same_version() {
    ...
    let mut runtime = KameoRuntime::with_node_id(NodeId("node-1".into()));

    let t1 = InMemoryTransport::new(NodeId("node-1".into()));
    let t2 = InMemoryTransport::new(NodeId("node-2".into()));

    t1.set_handshake_info(handshake_req("node-1", "0.2.0", "kameo")).await;
...
async fn try_connect_peer_accepted_different_app_versions() {
    ...
    t1.set_handshake_info(handshake_req_with_app("node-1", "0.2.0", "kameo", "2.3.1"))
        .await;
    t2.set_handshake_info(handshake_req_with_app("node-2", "0.2.0", "kameo", "2.3.0"))
        .await;
...
async fn try_connect_peer_rejected_incompatible_protocol() {
    ...
    t1.set_handshake_info(handshake_req("node-1", "0.2.0", "kameo")).await;
    t2.set_handshake_info(handshake_req("node-2", "1.0.0", "kameo")).await;
...
async fn try_connect_peer_accepted_same_major_different_minor() {
    ...
    t1.set_handshake_info(handshake_req("node-1", "0.2.0", "kameo")).await;
    t2.set_handshake_info(handshake_req("node-2", "0.3.0", "kameo")).await;
...
async fn try_connect_peer_handshake_request_uses_runtime_config() {
    let runtime = KameoRuntime::with_node_id(NodeId("my-node".into()))
