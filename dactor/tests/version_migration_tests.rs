...
use dactor::remote::{
    receive_envelope_body_versioned, MessageVersionHandler, WireEnvelope, WireHeaders,
};
...
fn v1_to_v2_migration_adds_priority_byte() {
    ...
    let result = receive_envelope_body_versioned(&envelope, &registry, &handlers, Some(2)).unwrap();
...
fn v2_to_v3_migration_passes_through() {
    ...
    let result = receive_envelope_body_versioned(&envelope, &registry, &handlers, Some(3)).unwrap();
    let body = result.downcast::<Vec<u8>>().unwrap();
...
fn receiver_version_none_skips_handler() {
    ...
    let result = receive_envelope_body_versioned(&envelope, &registry, &handlers, None).unwrap();
    let received = result.downcast::<Vec<u8>>().unwrap();
    assert_eq!(*received, body);
}
