...
///
/// dactor is an abstraction layer for actor frameworks. All transport-level
/// logic (TCP connections, gRPC channels, address resolution) belongs to the
/// actual provider/adapter, not to dactor. The `Transport` trait only
/// specifies how to send and receive messages once connectivity is
/// established.
///
/// - **Connection management** is provider-specific (the adapter decides
///   how and when to establish connections).
/// - **Version handshakes** are performed via system actors
///   ([`HandshakeRequest`]/[`HandshakeResponse`]) through the normal
///   message path, not via a dedicated transport method.
/// - **Address resolution** is handled by the adapter/provider before
///   messages are sent.
#[async_trait]
